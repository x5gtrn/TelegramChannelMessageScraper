...
class TelegramScraper:
    ...
    async def fetch_messages(self, channel_identifier: str) -> List[Dict[str, Any]]:
        """
        Fetch all admin messages from a channel.
        ...
        try:
            ...
            # Get total message count (approximate)
            # We'll use iter_messages which handles pagination automatically
            print("Counting messages...")
            
            messages_data = []
            admin_ids = set()

            # First pass: collect admin IDs
            print("Identifying admin users...")
            async for message in self.client.iter_messages(
                channel, limit=None, reverse=True, min_id=self.last_message_id
            ):
                if message.sender_id and message.sender_id not in admin_ids:
                    if await self.is_admin(channel, message.sender_id):
                        admin_ids.add(message.sender_id)
                        print(f"Found admin: {message.sender_id}")

            if not admin_ids:
                print("Warning: No admin users found. This might take longer.")

            # Second pass: collect admin messages
            ...
            with tqdm(total=total_count, desc="Processing messages") as pbar:
                async for message in self.client.iter_messages(
                    ...
                    if message.sender_id in admin_ids or (
                        message.sender_id and await self.is_admin(channel, message.sender_id)
                    ):
            ...
            print(f"\nTotal admin messages collected: {len(messages_data)}")
            ...
            return await self.fetch_messages(channel_identifier)
        except Exception as e:
    ...
    async def run(self, channel_identifier: str):
...
async def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python main.py <channel_username_or_id>")
        print("Example: python main.py my_channel")
        print("Example: python main.py 1234567890")
        sys.exit(1)
    ...
    await scraper.run(channel_identifier)
